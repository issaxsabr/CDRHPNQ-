...
const CacheModal: React.FC<CacheModalProps> = ({ 
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-100 flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200">
                <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-2xl">
                    <div className="flex items-center gap-2 text-slate-800"><Database className="w-5 h-5 text-indigo-500" /><h3 className="font-bold text-lg">Cache Local (24h)</h3></div>
                ...
                <div className="flex-1 overflow-y-auto p-0">
                    {cachedItems.length > 0 ? (
                        <table className="w-full text-left text-xs text-slate-600">
                            <thead className="bg-white sticky top-0 z-10 shadow-sm text-slate-400 uppercase tracking-wider text-[10px]">
                                <tr>
                                    <th className="px-4 py-3 w-10 text-center"><button onClick={onToggleSelectAllCache} className="hover:text-slate-800"><CheckSquare className="w-4 h-4"/></button></th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {cachedItems.map((item, idx) => (
                                    <tr key={idx} className={`transition-colors cursor-pointer ${selectedCacheKeys.has(item.term) ? 'bg-indigo-50' : 'hover:bg-slate-50'}`} onClick={() => onToggleSelectCacheItem(item.term)}>
