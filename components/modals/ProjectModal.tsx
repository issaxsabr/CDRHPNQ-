        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full w-[95vw] sm:w-full max-w-lg lg:max-w-3xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[75vh] sm:max-h-[85vh] animate-scale-in glass-indigo">
                <div className="p-5 border-b border-white/20 flex items-center justify-between rounded-t-2xl">
                    ...
                    <Button onClick={onClose} variant="ghost" size="icon" rounded="full" className="hover:bg-black/5 text-slate-400"><X className="w-5 h-5" /></Button>
                </div>
                                ...
                                <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                    <div className="flex items-center gap-1 mr-2 bg-white/80 rounded-lg border border-slate-200 p-1 shadow-sm">
                                        ...
                                        <Button 
                                            ...
                                            variant="ghost"
                                            size="icon"
                                            className="w-7 h-7 text-slate-400 hover:text-green-600 hover:bg-green-50"
                                    ...
                                    {activeProjectId !== p.id ? (
                                        <Button onClick={() => { onSelectProject(p.id); onClose(); }} variant="secondary" size="xs" rightIcon={<ArrowRight className="w-3 h-3"/>} className="hover:text-indigo-600">
                                            Ouvrir
                                        </Button>
                                    ) : (
                                    ...
                                    <Button onClick={(e) => onDeleteProject(p.id, e)} variant="ghost" size="icon" className="text-slate-400 hover:text-rose-600 hover:bg-rose-50" title="Supprimer">
