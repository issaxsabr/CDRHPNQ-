...
// Composant Cellule Éditable (Clean style)
const EditableCell = ({ 
    value, 
    onSave, 
    type = "text",
...
const ResultTable: React.FC<ResultTableProps> = ({ data, onUpdate, columnLabels }) => {
  ...
  const getLabelForEmail = (email: string, decisionMakers?: ContactPerson[]) => {
      ...
      if (maker) { 
          // Priorité au titre si disponible, sinon le nom
  ...
  const gridTemplateColumns = "18% 10% 12% 10% 14% 11% 7% 12% 6%";
  const ROW_HEIGHT = 80; // Compact height
  ...
  const Row = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    return (
      ...
      >
        ...
        {/* 3. CUSTOM FIELD */}
        <div className="px-3 py-2 h-full flex flex-col justify-center border-r border-transparent group-hover:border-slate-100">
            <div className="flex items-start gap-1 group/custom">
                <PenTool className="w-3 h-3 text-slate-300 mt-0.5 shrink-0 opacity-0 group-hover/custom:opacity-100 transition-opacity" />
                <div className="flex-1 min-w-0 text-[11px] text-slate-600">
                    <EditableCell 
                        value={item.customField || ""} 
                        onSave={(val) => onUpdate(index, 'customField', val)}
                        placeholder="..."
                        className=""
                    />
                </div>
            ...
            <div className="flex flex-col gap-1 items-start w-full">
                <div className="flex items-center gap-2 w-full group/email">
                    ...
                    <div className="flex-1 min-w-0 font-mono text-[10px] text-slate-700">
                        <EditableCell 
                            value={primaryEmail} 
                            onSave={(val) => onUpdate(index, 'email', val)}
                            type="email"
                            placeholder="Email..."
                        />
                    </div>
    ...
    <div className="w-full overflow-hidden rounded-xl border border-slate-200 shadow-xl shadow-slate-200/50 bg-white flex flex-col h-[650px] animate-fade-in-up">
        <div className="flex-1 flex flex-col overflow-x-auto">
            <div style={{ minWidth: 1400 }} className="flex flex-col h-full">
                {/* HEADER */}
                <div className="grid bg-slate-50 text-[10px] uppercase tracking-wider font-semibold text-slate-500 border-b border-slate-200 z-10 shrink-0 pr-2" style={{ gridTemplateColumns }}>
                    ...
                    <div className="px-3 py-3 border-r border-slate-200/50 bg-indigo-50/50 text-indigo-600">{columnLabels.customField}</div>
